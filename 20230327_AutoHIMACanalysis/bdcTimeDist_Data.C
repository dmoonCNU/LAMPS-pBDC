...
void bdcTimeDist_Data(int locano=2, int minip=-1, int maxip=10, int tdopt=1, int hitopt=1, int chopt=1, int centopt=1, int RunNumber=3067, int ltopt=1, int ltch=15){
	...
	diftime4 = 1850 + ht0val->GetBinContent(4);
	    ...
	    if (locano==3) {
    diftime1 = 1950 + ht0val->GetBinContent(1);
    diftime2 = 1950 + ht0val->GetBinContent(2);
    diftime3 = 1950 + ht0val->GetBinContent(3);
    diftime4 = 1950 + ht0val->GetBinContent(4);
    }
	 ...
	 */
	int v1ch=0;
	double minchis=10000.0;
	for(int j=13;j<18;j++) {
		TimeInt1Ch[0][j]->SetLineColor(kRed+2);
		TimeInt1Ch[1][j+1]->SetLineColor(kBlue+2);
		...
		TimeInt1Ch[2][j]->Draw("same HIST");
		TimeInt1Ch[3][j]->Draw("same HIST");	
		canch->SaveAs(Form("plot_time_dist_X_X1v1Ch%dline.png",j));
		double tmpchis=0.0;
		for (int k=0;k<159;k++) {
			TH1D* htmp = new TH1D("htmp","",4,0,4);
			htmp->SetBinContent(1,TimeInt1Ch[0][j]->GetBinContent(k+1));
			htmp->SetBinContent(2,TimeInt1Ch[1][j+1]->GetBinContent(k+1));
			htmp->SetBinContent(3,TimeInt1Ch[2][j]->GetBinContent(k+1));
			htmp->SetBinContent(4,TimeInt1Ch[3][j]->GetBinContent(k+1));
			//			std::cout << htmp->GetBinContent(1) << " , " << htmp->GetBinContent(2) << " , " << htmp->GetBinContent(3) << " , " << htmp->GetBinContent(4) << std::endl;
			TF1* fline = new TF1("fline","[0]+0*x",0,4);
			htmp->Fit(fline,"RQM");
			tmpchis+=fline->GetChisquare();
			htmp->Clear();
		}
		std::cout << "v1 CH " << j << " , chi-square: " << tmpchis << std::endl;
		if (tmpchis<=minchis) {v1ch=j;minchis=tmpchis;}
	}

	int v2ch=0;
	double minchis2=10000.0;
	for(int j=13;j<18;j++) {
		TimeInt1Ch[0][j]->SetLineColor(kRed+2);
		TimeInt1Ch[1][j]->SetLineColor(kBlue+2);
		TimeInt1Ch[2][j]->SetLineColor(kOrange+2);
		TimeInt1Ch[3][j-1]->SetLineColor(kGreen+2);
		TimeInt1Ch[0][j]->Draw("HIST");
		TimeInt1Ch[1][j]->Draw("same HIST");
		TimeInt1Ch[2][j]->Draw("same HIST");
		TimeInt1Ch[3][j-1]->Draw("same HIST");	
		canch->SaveAs(Form("plot_time_dist_X_X1v2Ch%dline.png",j));
		double tmpchis=0.0;
		for (int k=0;k<159;k++) {
			TH1D* htmp = new TH1D("htmp","",4,0,4);
			htmp->SetBinContent(1,TimeInt1Ch[0][j]->GetBinContent(k+1));
			htmp->SetBinContent(2,TimeInt1Ch[1][j]->GetBinContent(k+1));
			htmp->SetBinContent(3,TimeInt1Ch[2][j]->GetBinContent(k+1));
			htmp->SetBinContent(4,TimeInt1Ch[3][j-1]->GetBinContent(k+1));
			//			std::cout << htmp->GetBinContent(1) << " , " << htmp->GetBinContent(2) << " , " << htmp->GetBinContent(3) << " , " << htmp->GetBinContent(4) << std::endl;
			TF1* fline = new TF1("fline","[0]+0*x",0,4);
			htmp->Fit(fline,"RQM");
			tmpchis+=fline->GetChisquare();
		...
		std::cout << "v2 CH " << j << " , chi-square: " << tmpchis << std::endl;
		if (tmpchis<=minchis2) {v2ch=j;minchis2=tmpchis;}
	}
	...
	std::cout << "lt function (v1) : v1ch - " << v1ch << " , chisquare - " << minchis << std::endl; 
	std::cout << "lt function (v2) : v2ch - " << v2ch << " , chisquare - " << minchis2 << std::endl; 


	//	ltch=17;

	if (ltopt==1) {
		TimeInt1[0]=(TH1D*)TimeInt1Ch[0][ltch]->Clone();
		TimeInt1[1]=(TH1D*)TimeInt1Ch[1][ltch+1]->Clone();
		TimeInt1[2]=(TH1D*)TimeInt1Ch[2][ltch]->Clone();
		TimeInt1[3]=(TH1D*)TimeInt1Ch[3][ltch]->Clone();
		std::cout << "lt function (decided) : v1ch - " << ltch << std::endl; 
	}
	else if (ltopt==2) {
		TimeInt1[0]=(TH1D*)TimeInt1Ch[0][ltch]->Clone();
		TimeInt1[1]=(TH1D*)TimeInt1Ch[1][ltch]->Clone();
		TimeInt1[2]=(TH1D*)TimeInt1Ch[2][ltch]->Clone();
		...
		if (minchis<=minchis2) {
			...
			TimeInt1[2]=(TH1D*)TimeInt1Ch[2][v1ch]->Clone();
			TimeInt1[3]=(TH1D*)TimeInt1Ch[3][v1ch]->Clone();
			std::cout << "v1 is selected !!!" << std::endl;
		}
		else {
			TimeInt1[0]=(TH1D*)TimeInt1Ch[0][v2ch]->Clone();
			TimeInt1[1]=(TH1D*)TimeInt1Ch[1][v2ch]->Clone();
			TimeInt1[2]=(TH1D*)TimeInt1Ch[2][v2ch]->Clone();
			TimeInt1[3]=(TH1D*)TimeInt1Ch[3][v2ch-1]->Clone();
			std::cout << "v2 is selected !!!" << std::endl;
		}
	}
