...
bool tangent_ = true; // fitting algorithm (true trangential method, false linear fit method)
...
void trackFinderX2(float dataCh1X1, float dataDl1X1, float dataCh1X2, float dataDl1X2, float dataCh2X1, float dataDl2X1, float dataCh2X2, float dataDl2X2, float dataDt1X1, float dataDt1X2, float dataDt2X1, float dataDt2X2, int ievt, int tracknum, double *pars, double *dts, TCanvas *ca, bool savepng, int angleCut_, double minang, double maxang);
...
void bdcTrackFinderData_CNU_Cosmic_getResolution(const bool savepng=true, bool corr_=true, double nsigma=1.5, int angleCut_=1, double minang=-0.467-1.0, double maxang=-0.467+1.0)
{
	...
	TF1* fgaus;
	double ang0mean = 0.0;
	double ang0RMS = 0.001;
	if (angleCut_>0) {
	fang = new TFile("anaAngleResultang0.root","READ");
	hang0 = (TH1F*)fang->Get("hAngX");
	fgaus = (TF1*)fang->Get("gaus");
	ang0mean = hang0->GetMean();
	ang0RMS = hang0->GetRMS();
	std::cout << "hang0 : " << ang0mean << " , " << ang0RMS << std::endl;
	ang0mean = fgaus->GetParameter(1);
	ang0RMS = fgaus->GetParameter(2);
	std::cout << "gaus : " << ang0mean << " , " << ang0RMS << std::endl;
	}


	TFile *data = new TFile("tTrackTree_BDC_Run_Cosmic_CNU_1430_v16.root","READ");
	...
	TFile *outfile = new TFile("bdcAnaTrack_Data_CNU_Cosmic_Run_getResolution_v16.root","RECREATE");
	...
	cormin1=hcorr->GetBinContent(1)-nsigma*(hcorr->GetBinError(1));
	cormax1=hcorr->GetBinContent(1)+nsigma*(hcorr->GetBinError(1));
	cormin2=hcorr->GetBinContent(2)-nsigma*(hcorr->GetBinError(2));
	cormax2=hcorr->GetBinContent(2)+nsigma*(hcorr->GetBinError(2));
	cormin3=hcorr->GetBinContent(3)-nsigma*(hcorr->GetBinError(3));
	cormax3=hcorr->GetBinContent(3)+nsigma*(hcorr->GetBinError(3));

	std::cout << "##### Correlation information #####" << std::endl;
	std::cout << "nsigma : " << nsigma << std::endl;
	std::cout << cormin1 << " ~ " << cormax1 << std::endl;
	std::cout << cormin2 << " ~ " << cormax2 << std::endl;
	std::cout << cormin3 << " ~ " << cormax3 << std::endl;
	std::cout << std::endl;
	...
	TH1F *hAngX = new TH1F("hAngX",";Angle(#theta) at X;counts",2000,-100,100);
	TH1F *hAngY = new TH1F("hAngY",";Angle(#theta) at Y;counts",2000,-100,100);
	...
	TH2F *hBeamSpot4 = new TH2F("hBeamSpot4","Beam Spot at Target;X (mm);Y (mm)",250,-500,500,250,-500,500);
	...
	TH1F *hPosX4 = new TH1F("hPosX4","Beam Spot at X;X (mm);Counts",200,0,200);
	...
	TH1F *hPosY4 = new TH1F("hPosY4","Beam Spot at Y;Y (mm);Counts",200,0,200);
	...
	for(int iw = 0; iw < nwires; iw++){
		...
		wireX4[iw] = iw*5.0 + Xwirestart+5.0+cal4; // y'
		wireY1[iw] = FLH;
	...
	for(int ievt = 0; ievt < maxevt; ievt++){
		...
		double XPos4 = 0.0; // X position of Target
		...
		double YPos4 = 0.0; // Y position of Target
										...
										for(int Y22=0;Y22<nY22;Y22++)
										{
											...
											if(datadiff2==0 || datadiff2==-1)
											{
												...
												if(fulltracking){
													...
													if(corr_ == true) {
														if(tmax1 > cormax1) continue;
														if(tmax1 < cormin1) continue;
														if(tmax2 > cormax2)  continue;
														if(tmax2 < cormin2)  continue;
														if(tmax3 > cormax3)  continue;
														if(tmax3 < cormin3)  continue;
													}
													...
													trackFinderY2(dataCh1Y1[Y11], dataDl1Y1[Y11], dataCh1Y2[Y12], dataDl1Y2[Y12], dataCh2Y1[Y21], 
															...
															ievt, tracknum, pars, dts, savepng);
													if(tracknum==0)
													{
														Ygrad[0] = pars[0];
														Yc[0] = pars[1];
		...
		if ( trkNumX>0 || trkNumY>0 ) {
			...
			if(trkNumX>0){
				...
				if(angleCut_!=0 && (!(minang<angle && angle<maxang))) continue;
...
void trackFinderY2(float dataCh1Y1, float dataDl1Y1, float dataCh1Y2, float dataDl1Y2, float dataCh2Y1, float dataDl2Y1, float dataCh2Y2, float dataDl2Y2, float dataDt1Y1, float dataDt1Y2, float dataDt2Y1, float dataDt2Y2, int ievt, int tracknum, double *pars, double *dts, bool savepng){
	...
	for(int iw = 0; iw < nwires; iw++){
		...
		wireX4[iw] = iw*5.0 + Ywirestart+5.0+cal4; // y'
		wireY1[iw] = FLH+10;
		wireY2[iw] = FLH+15;
		//wireY1[iw] = FLH+10;
		//wireY2[iw] = FLH+15;
	...
	double ysec1 = -lineSeed->GetParameter(1)/lineSeed->GetParameter(0); // Jain added
	...
	double ysec2 = -lineSeed1->GetParameter(1)/lineSeed1->GetParameter(0); // Jain added
	...
	for(int i = 0; i < nlayer; i++){
		...
		if(tangent_ == true) distances1[i] = fabs(dist) - driftlth[i];;
		if(tangent_ == false) distances1[i] = fabs(lineSeed->Eval(seedy[i])-wireposx[i]) - driftlth[i];;
		// #### Jaine's code End ####
	...
	for(int i = 0; i < nlayer; i++){
		...
		double dist = distance(wireposx[i],wireposy[i],trpars);
		if(tangent_ == true) distances3[i] = fabs(dist) - driftlth[i];;
		if(tangent_ == false) distances3[i] = fabs(track2->Eval(seedy[i])-wireposx[i]) - driftlth[i];;
	...
	//track_3p->SetLineColor(kTeal+1);
	bdcTrack_3p->SetLineStyle(2);
	track_3p->SetLineStyle(5);
	bdcTrack->Draw("same");
	bdcTrack_3p->Draw("same");
	track->Draw("same");
	track_3p->Draw("same");

	double target_pos = target_dist - seedy[0]; 

	pars[12] = bdcTrack->GetX(seedy[3]); 
	pars[13] = bdcTrack->GetX((seedy[0]+seedy[3])/2);
	pars[14] = bdcTrack->GetX(seedy[0]);
	pars[15] = bdcTrack->GetX(target_pos);
...
void trackFinderX2(float dataCh1X1, float dataDl1X1, float dataCh1X2, float dataDl1X2, float dataCh2X1, float dataDl2X1, float dataCh2X2, float dataDl2X2, float dataDt1X1, float dataDt1X2, float dataDt2X1, float dataDt2X2, int ievt, int tracknum, double *pars, double *dts, TCanvas *ca, bool savepng, int angleCut_, double minang, double maxang){
	...
	for(int l = 0; l < nlayer; l++){
		if(driftlth[l] < 0) driftlth[l] = 0.01;
	}
	...
	if(angleCut_!=0 && (!(minang<angle && angle<maxang))) {std::cout << "angle cut failed!!" << std::endl;pars[20]=0;return;}
