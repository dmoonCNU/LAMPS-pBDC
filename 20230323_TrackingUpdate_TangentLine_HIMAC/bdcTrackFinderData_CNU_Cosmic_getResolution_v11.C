...
void trackFinderX2(float dataCh1X1, float dataDl1X1, float dataCh1X2, float dataDl1X2, float dataCh2X1, float dataDl2X1, float dataCh2X2, float dataDl2X2, float dataDt1X1, float dataDt1X2, float dataDt2X1, float dataDt2X2, int ievt, int tracknum, double *pars, double *dts, TCanvas *ca);
...
void bdcTrackFinderData_CNU_Cosmic_getResolution_v11()
{
	...
	TFile *outfile = new TFile("bdcAnaTrack_Data_CNU_Cosmic_Run_getResolution_v11.root","RECREATE");
...
*/
	...
	grd1 = 1.0/lineSeed->GetParameter(0);
	ysec1 = -lineSeed->GetParameter(1)/lineSeed->GetParameter(0); // Jain added
	if(atan(grd1)*180/pi > 0) angle1 = 90-atan(grd1)*180/pi;
	if(atan(grd1)*180/pi < 0) angle1 = -90-atan(grd1)*180/pi;
	}
	...
	grd2 = 1.0/lineSeed1->GetParameter(0);
	ysec2 = -lineSeed1->GetParameter(1)/lineSeed1->GetParameter(0); // Jain added
	if(atan(grd2)*180/pi > 0) angle2 = 90-atan(grd2)*180/pi;
	if(atan(grd2)*180/pi < 0) angle2 = -90-atan(grd2)*180/pi;
	}
