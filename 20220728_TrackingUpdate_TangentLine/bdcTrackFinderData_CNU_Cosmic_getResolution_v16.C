...
// #### Jain code Start ####
double distance(double x,double y,const double *par) {
  // ax +  by + c = 0   ~ (x0,y0)
  // distnace = |ax0+by0+c|/sqrt(a^2+b^2)
  double d = fabs(par[0]*x - y + par[1]) / TMath::Sqrt(par[0]*par[0]+1) ;
  //  coust<< d << endl;
  return d;
}
...
void trackFinderX3(float dataCh1X1, float dataDl1X1, float dataCh1X2, float dataDl1X2, float dataCh2X1, float dataDl2X1, float dataCh2X2, float dataDl2X2, float dataDt1X1, float dataDt1X2, float dataDt2X1, float dataDt2X2, int ievt, int tracknum, double *pars, double *dts);
...
void trackFinderY3(float dataCh1Y1, float dataDl1Y1, float dataCh1Y2, float dataDl1Y2, float dataCh2Y1, float dataDl2Y1, float dataCh2Y2, float dataDl2Y2, float dataDt1Y1, float dataDt1Y2, float dataDt2Y1, float dataDt2Y2, int ievt, int tracknum, double *pars, double *dts);

// create Track tree to keep track information, gradient, intersection, event time
// This version is CNU cosmic data analysis on 20220504
void bdcTrackFinderData_CNU_Cosmic_getResolution_v16()
{
  ...
  TFile *data = new TFile("tTrackTree_BDC_Run_Cosmic_CNU_1430_v14.root","READ");
  ...
  myTree->SetBranchAddress("dur_sec1",&dur_sec1, &b_dur_sec1);
  myTree->SetBranchAddress("dur_sec2",&dur_sec2, &b_dur_sec2);
  myTree->SetBranchAddress("dur_sec3",&dur_sec3, &b_dur_sec3);
  myTree->SetBranchAddress("dur_sec4",&dur_sec4, &b_dur_sec4);
  myTree->SetBranchAddress("EvtTime1",&EvtTime1, &b_EvtTime1);
  myTree->SetBranchAddress("EvtTime2",&EvtTime2, &b_EvtTime2);
  myTree->SetBranchAddress("EvtTime3",&EvtTime3, &b_EvtTime3);
  myTree->SetBranchAddress("EvtTime4",&EvtTime4, &b_EvtTime4);
  ...
  TFile *outfile = new TFile("bdcAnaTrack_Data_CNU_Cosmic_Run_getResolution_20220601.root","RECREATE");
  ...
  trkTree->Branch("EvtTime",&EvtTime,"EvtTime/I");
  trkTree->Branch("dur_sec",&dur_sec,"dur_sec/D");
  trkTree->Branch("EvtTimeDif",&EvtTimeDif,"EvtTimeDif/I");
  trkTree->Branch("dur_secDif",&dur_secDif,"dur_secDif/D");
  trkTree->Branch("EvtTag",&EvtTag,"EvtTag/I");
  ...
  TH2F *hAngDifX = new TH2F("hAngDifX",";angle (deg);difference (mm)",120,-60,60,300,-3.0,3.0);
  TH2F *hAngDifY = new TH2F("hAngDifY",";angle (deg);difference (mm)",120,-60,60,300,-3.0,3.0);

  TH2F *hChiDifX = new TH2F("hChiDifX",";chisqaure;difference (mm)",100,0,10,300,-3.0,3.0);
  TH2F *hChiDifY = new TH2F("hChiDifY",";chisqaure;difference (mm)",100,0,10,300,-3.0,3.0);


  TH1D* TimeMax11 = new TH1D("TimeMax11",";time;Counts",500,0,500);
  TH1D* TimeMax21 = new TH1D("TimeMax21",";time;Counts",500,0,500);
  TH1D* DriftVel11 = new TH1D("DriftVel11",";Velocity (#mum/ns);Counts",100,0,100);
  TH1D* DriftVel21 = new TH1D("DriftVel21",";Velocity (#mum/ns);Counts",100,0,100);
  ...
  for(int ievt = 0; ievt < maxevt; ievt++){
    ...
    cout<<"Stage 1, nX11 : "<<nX11<<" nX12 : "<<nX12<<" nX21 : "<<nX21<<" nX22 : "<<nX22<<endl;

    Event = ievt;
    EvtTime = 0.0; // should be updated by time information
                ...
                for(int Y21 = 0;Y21<nY21;Y21++)
                {
                  for(int Y22=0;Y22<nY22;Y22++)
                  {
                    ...
                    if(datadiff2>=0 && datadiff2<=1)
                    {
                      ...
                      double pars[11] = {0.0}; // distances at each layer
                      double pars1[11] = {0.0}; // distances at each layer
                      double dts[6] = {0.0};
                      double dts1[6] = {0.0};
                      if(fulltracking){
                        ...
                        if(tmax1 > 110) continue;
                        if(tmax1 < 60) continue;
                        if(tmax2 > 110)  continue;
                        if(tmax2 < 60)  continue;
                        if(tmax3 > 110)  continue;
                        if(tmax3 < 60)  continue;
                        ...
                        /*
                        if(tracknum!=0 && rediff > fabs(pars[2])+fabs(pars[3])+fabs(pars[4])+fabs(pars[5]))
                        {
                          Ygrad[0] = pars[0];
                          Zgrad_Y[0] = 1/Ygrad[0];
                          Yc[0] = pars[1];
                          Zc_Y[0] = -pars[1]/Ygrad[0];
                          rediff = fabs(pars[2])+fabs(pars[3])+fabs(pars[4])+fabs(pars[5]);
                          goodtrack = tracknum;
                          YCH11 = dataCh1Y1[Y11];
                          YCH12 = dataCh1Y2[Y12];
                          YCH21 = dataCh2Y1[Y21];
                  ...
                  for(int X22=0;X22<nX22;X22++)
                  {
                    ...
                    if(datadiff2>=0 && datadiff2<=1)
                    {
                      ...
                      if(fulltracking){
                        ...
                        if(tmax1 > 105) continue;
                        if(tmax1 < 65) continue;
                        if(tmax2 > 105)  continue;
                        if(tmax2 < 65)  continue;
                        //if(tmax3 > 105)  continue;
                        //if(tmax3 < 65)  continue;
    ...
    if ( trkNumX>0 || trkNumY>0 ) {
      ...
      // std::cout << "########## ievt : " << ievt << " " << dur_sec << std::endl;
      ievtpre=ievt;
  ...
  for(int ievt = 0;ievt<maxevt;ievt++){
    if(retrack[ievt][4]>0)
    {
      //std::cout<<"Event : "<<ievt <<"  TrackNum : "<< retrack[ievt][4] <<" diff : "<<retrack[ievt][0]<<"  "<<retrack[ievt][1]<<"  "<<retrack[ievt][2]<<"  "<<retrack[ievt][3]<<std::endl;
    }
  }
  */
  outfile->cd();
...
void trackFinderY2(float dataCh1Y1, float dataDl1Y1, float dataCh1Y2, float dataDl1Y2, float dataCh2Y1, float dataDl2Y1, float dataCh2Y2, float dataDl2Y2, float dataDt1Y1, float dataDt1Y2, float dataDt2Y1, float dataDt2Y2, int ievt, int tracknum, double *pars, double *dts){
  cout<<"Start track with y axis !!! "<<ievt<<endl;
  ...
  ysec1 = -lineSeed->GetParameter(1)/lineSeed->GetParameter(0);
  ...
  ysec2 = -lineSeed1->GetParameter(1)/lineSeed1->GetParameter(0);
  if(atan(grd2)*180/pi > 0) angle2 = 90-atan(grd2)*180/pi;
  if(atan(grd2)*180/pi < 0) angle2 = -90-atan(grd2)*180/pi;

  cout<<"Angle with 4p : "<<angle1<<endl;
  cout<<"Angle with 3p : "<<angle2<<endl;


  // #### Add Jain's Code Start ####
  double x,y;
  TGraph* gr = new TGraph(4);

  gr->SetPoint(0,wireposx[0],wireposy[0]);
  gr->SetPoint(1,wireposx[1],wireposy[1]);
  gr->SetPoint(2,wireposx[2],wireposy[2]);
  gr->SetPoint(3,wireposx[3],wireposy[3]);


  double r[4] = {driftlth[0], driftlth[1], driftlth[2], driftlth[3]};
  ...
  auto chi2Function = [&](const double *par) {
    ...
    double *y = gr->GetY();
    for (int i=0;i<np;i++) {
      double dist = distance(x[i],y[i], par);
      f += (dist-r[i])*(dist-r[i]);
      cout<<"check   "<<"x["<<i<<"] : "<<x[i];
      cout<<"   "<<"y["<<i<<"] : "<<y[i]<<endl;
    }
  ...
  if (!ok) {
    Error("line2Dfit","Line2D Fit failed");
  }
  ...
  TF1 *track = new TF1("Track","[0]*x+[1]",-1000000,1000000);
  TF1 *track2 = new TF1("Track2","[0]*x+[1]",-1000000,1000000);

  track->SetParameter(0,result.Parameter(0));
  track->SetParameter(1,result.Parameter(1));

  track2->SetParameter(0,1.0/track->GetParameter(0));
  track2->SetParameter(1,-track->GetParameter(1)/track->GetParameter(0));
  ...
  for(int i = 0; i < nlayer; i++){
    ...
    distances1[i] = fabs(track2->Eval(seedy[i])-wireposx[i]) - driftlth[i];;
  ...
  for(int i = 0; i < nlayer; i++){
    ...
    distances3[i] = fabs(track2->Eval(seedy[i])-wireposx[i]) - driftlth[i];;
...
void trackFinderX2(float dataCh1X1, float dataDl1X1, float dataCh1X2, float dataDl1X2, float dataCh2X1, float dataDl2X1, float dataCh2X2, float dataDl2X2, float dataDt1X1, float dataDt1X2, float dataDt2X1, float dataDt2X2, int ievt, int tracknum, double *pars, double *dts){
  ...
  for(int iw = 0; iw < nwires; iw++){
    ...
    /*
    wireX1[iw] = iw*5.0 + Xwirestart+0.+cal1; // y
    wireX2[iw] = iw*5.0 + Xwirestart+2.5	+cal2; // y'
    wireX3[iw] = iw*5.0 + Xwirestart+0.+cal3; // y
    wireX4[iw] = iw*5.0 + Xwirestart+2.5	+cal4; // y'
    */
  ...
  gr1->SetPoint(0,wireposx[0],wireposy[0]);
  gr1->SetPoint(1,wireposx[1],wireposy[1]);
  gr1->SetPoint(2,wireposx[3],wireposy[3]);


  double r[4] = {driftlth[0], driftlth[1], driftlth[2], driftlth[3]};
  double r1[3] = {driftlth[0], driftlth[1], driftlth[3]};
  ...
  auto chi2Function1 = [&](const double *par) {
    //minimisation function computing the sum of squares of residuals
    // looping at the graph points
    int np = gr1->GetN();
    double f = 0;
  ...
  // with 3 points
  ROOT::Math::Functor fcn1(chi2Function1,2);
  ROOT::Fit::Fitter  fitter1;


  double pStart1[2] = {grd1,ysec1};
  fitter1.SetFCN(fcn1, pStart1);
  fitter1.Config().ParSettings(0).SetName("a");
  fitter1.Config().ParSettings(1).SetName("b");

  // do the fit
  bool ok1 = fitter1.FitFCN();
  if (!ok1) {
    Error("line2Dfit","Line2D Fit failed");
  }
  ...
  pars[0] = track->GetParameter(0);
  pars[1] = track->GetParameter(1);

  track2->SetParameter(0,1.0/track->GetParameter(0));
  track2->SetParameter(1,-track->GetParameter(1)/track->GetParameter(0));


  TF1 *track_3p = new TF1("Track_3p","[0]*x+[1]",-1000000,1000000);
  TF1 *track2_3p = new TF1("Track2_3p","[0]*x+[1]",-1000000,1000000);

  track_3p->SetParameter(0,result1.Parameter(0));
  track_3p->SetParameter(1,result1.Parameter(1));

  track2_3p->SetParameter(0,1.0/track_3p->GetParameter(0));
  track2_3p->SetParameter(1,-track_3p->GetParameter(1)/track_3p->GetParameter(0));
  ...
  for(int i = 0; i < nlayer; i++){
    ...
    distances[i] = recodriftlth[i] - driftlth[i];
    double trpars[2] = {0.0};
    trpars[0] = track->GetParameter(0);
    trpars[1] = track->GetParameter(1);
    double dist = distance(wireposx[i],wireposy[i],trpars);
    distances1[i] = fabs(dist) - driftlth[i];;
    //distances1[i] = fabs(track2->Eval(seedy[i])-wireposx[i]) - driftlth[i];;
  ...
  for(int i = 0; i < nlayer; i++){
    ...
    trpars[0] = track->GetParameter(0);
    trpars[1] = track->GetParameter(1);
    //trpars[0] = track_3p->GetParameter(0);
    //trpars[1] = track_3p->GetParameter(1);
    double dist = distance(wireposx[i],wireposy[i],trpars);
    //distances3[i] = fabs(dist) - driftlth[i];;
  ...
  bdcTrack->SetLineStyle(1);
  bdcTrack->SetLineWidth(3);
  track->SetLineColor(kBlue+2);
  track_3p->SetLineColor(kRed+2);
  track->Draw("same");
  track_3p->Draw("same");
  //bdcTrack->Draw("same");
  if(savepng)
...
void trackFinderY3(float dataCh1Y1, float dataDl1Y1, float dataCh1Y2, float dataDl1Y2, float dataCh2Y1, float dataDl2Y1, float dataCh2Y2, float dataDl2Y2, float dataDt1Y1, float dataDt1Y2, float dataDt2Y1, float dataDt2Y2, int ievt, int tracknum, double *pars, double *dts){
  cout<<"Start track with y axis !!! "<<ievt<<endl;
  TH2F *hPadY = new TH2F("hPadY",";y (mm) ;z (mm) ",200,0.0,165.0,165,900,1000.0);
  hPadY->GetYaxis()->CenterTitle();
  hPadY->GetXaxis()->CenterTitle();
  TH2F *hPadY1 = new TH2F("hPadY1",";y (mm) ;z (mm) ",200,0.0,165.0,165,0.0,100.0);
  hPadY1->GetYaxis()->CenterTitle();
  hPadY1->GetXaxis()->CenterTitle();

  TCanvas *c1 = new TCanvas("c1","",1500,650);
  c1->Divide(1,2);
  c1->cd(1);
  hPadY->Draw();
  int firedChX[4] = {0};
  ...
  if((wireposx[1]-wireposx[0])>0){ seedx[1]=wireposx[1]-driftlth[1]; seedx[0]=wireposx[0]+driftlth[0]; }
  else { seedx[1]=wireposx[1]+driftlth[1];seedx[0]=wireposx[0]-driftlth[0]; }

  if((wireposx[3]-wireposx[2])>0){ seedx[3]=wireposx[3]-driftlth[3]; seedx[2]=wireposx[2]+driftlth[2]; }
  else { seedx[3]=wireposx[3]+driftlth[3]; seedx[2]=wireposx[2]-driftlth[2]; }



  TF1 *lineSeed = new TF1("lineSeed","[0]*x+[1]",-100000000.0,100000000.0);
  lineSeed->SetLineColor(kBlue+1);
  lineSeed->SetLineStyle(4);
  lineSeed->SetLineWidth(4);
  ...
  pars[0]=lineSeed1->GetParameter(0);
  pars[1]=lineSeed1->GetParameter(1);

  // reconstructed drift length
  double recodriftlth[4];
  for(int i = 0; i < nlayer; i++){
    recodriftlth[i] = dist(wireposx[i],wireposy[i], 2,lineSeed1->Eval(2),4,lineSeed1->Eval(4));
    //pars[i+2] = recodriftlth[i] - driftlth[i]; // for resolution
  }
  ...
  double distances[4];
  for(int i = 0; i < nlayer; i++){
    distances[i] = lineSeed1->GetX(seedy[i]) - seedx[i];
    cout<<"distances["<<i<<"] : "<<distances[i]<<endl;
    pars[i+2] = distances[i]; // resolution
  }

  pars[6] = lineSeed1->GetX(seedy[2]) - seedx[2];
  ...
  //c1->SaveAs(Form("figs/plot_track_Y3_%d_%d.png",ievt,tracknum));
  //
  if(savepng)
  {
    c1->SaveAs(Form("figs/plot_track_Y3_%d_%d.png",ievt,tracknum));
...
void trackFinderX3(float dataCh1X1, float dataDl1X1, float dataCh1X2, float dataDl1X2, float dataCh2X1, float dataDl2X1, float dataCh2X2, float dataDl2X2, float dataDt1X1, float dataDt1X2, float dataDt2X1, float dataDt2X2, int ievt, int tracknum, double *pars, double *dts){
  cout<<"Start track with x axis !!! "<<ievt<<endl;
  TH2F *hPadX = new TH2F("hPadX",";x (mm) ;z (mm) ",175,-10.0,165,165,900,1000.0);
  hPadX->GetYaxis()->CenterTitle();
  hPadX->GetXaxis()->CenterTitle();
  TH2F *hPadX1 = new TH2F("hPadX1",";x (mm) ;z (mm) ",175,-10,165,165,0.0,100.0);
  hPadX1->GetYaxis()->CenterTitle();
  hPadX1->GetXaxis()->CenterTitle();
  ...
  if(savepng)
  {
    c1->SaveAs(Form("figs/plot_track_X3_%d_%d.png",ievt,tracknum));
  } 
  if(nodrawcan)
  {
    delete c1;
